...
I tested 50,838 codons for Sunflower, so significance threshold= 0.05/50838 <- way too stringent for this dataset. Using BH/FDR correction
...
# I also edited the R function due to realizing the alignment column *included* the reference genome (ie unmasked alignment) and therefore this had to be taken into account when determining whether reference allele was found in other genomes
...
Result <- TolvDel_sites("/scratch/eld72413/SAM_seq/BAD_Mut_Files/Results/Sunflower_SAM_Combined_Report.txt", "/scratch/eld72413/SAM_seq/VeP/SAM_SNP_Final_BiallelicNorm", 0.05, 10, 1, "LogisticP_Masked")

aggregate(Result$VariantID, by=list(Result$Result,
	Result$Refderived, Result$Altderived, Result$Result_Allele), length) # only difference is NAs not removed
Result[which(is.na(Result$Alignment)),]
length(Result$VariantID) # 645,213

# remove NAs (the 2 with 'NA' in VariantID column were removed in the merge)
Result <- Result[-(which(is.na(Result$Alignment))),]
length(Result$VariantID) # 645,208

# need to remove duplicate sites
length((unique(Result$VariantID))) #641,514 (3694 duplicates)

Result_split <- split(Result, as.factor(Result$Result_Allele))

# how many duplicated within categories?
lapply(Result_split, function(x) {length(x$VariantID)}) # alt deleterious: 76095, ref deleterious 11796, tolerated 557317
lapply(Result_split, function(x) {length(unique(x$VariantID))}) # alt deleterious: 76016, ref deleterious 11796, tolerated 554460
lapply(Result_split, function(x) {length(x[duplicated(x$VariantID),"VariantID"])}) # alt deleterious: 79, ref deleterious: 0, tolerated 2857

# same allele - derived in alternate vs. reference
length(which(Result_split$Alternate_deleterious$VariantID %in% 
  Result_split$Reference_deleterious$VariantID)) # 18

# identify positions that are deleterious for one gene and tolerated for another:
dups1 <- Result_split$Alternate_deleterious[which(Result_split$Alternate_deleterious$VariantID %in% 
  Result_split$Tolerated$VariantID),"VariantID"]
length(dups1) # 666

dups2 <- Result_split$Reference_deleterious[which(Result_split$Reference_deleterious$VariantID %in% 
  Result_split$Tolerated$VariantID),"VariantID"]
length(dups2) # 74

dups <- union(dups1, dups2) # 740

# mark the duplicates:
Result$Result_Allele[Result$Result_Allele=="Tolerated" &
                      Result$VariantID %in% dups] <- "Tolerated_duplicateDel"

# check
aggregate(Result$VariantID, by=list(Result$Result_Allele), length)
# 556577 positions now in tolerated (in vcf this would be 556577-2857=553,720)

write.table(Result, "/scratch/eld72413/SAM_seq/BAD_Mut_Files/Results/dsnp_data.table", sep = "\t", quote=FALSE, row.names=FALSE)
```
