...
unfolded_sfs <- read.table("/Volumes/GoogleDrive/My Drive/Active Projects/DelMutation/Results/SFS/DerivedFreq_Bins.txt",
                         sep="\t", header = T)

unfolded_sfs$Annotation <- factor(folded_sfs$Annotation, levels=c("NonCodingNodups", "SynonymousNodups",
                                                                "Tolerated", "MissenseOther", "AllDel", 
                                                                "StartStopLostGainednoDups", "SpliceAcceptorDonorNodups"))

unfolded_sfs_subset <- subset(unfolded_sfs, Annotation %in% AnnotationsToKeep)

p2 <- ggplot(unfolded_sfs_subset, aes(x=breaks, y=prop, fill=Annotation))
...
ggsave("/Volumes/GoogleDrive/My Drive/Active Projects/DelMutation/Manuscript/Sunflower_MutationLoad_Manuscript/Sunflower_MutationLoad_v1/RawFigs/UnFoldedSFS.pdf",
       width = 10, height = 5)
